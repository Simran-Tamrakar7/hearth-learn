...
- **Listing:** `src/app/manuals/registry.ts` (`KEPT_BUILTIN_SLUGS`). Those four always show on `/manuals`; localStorage hide is only for user-added manuals.
...
Full Content prefers an ordered `blocks[]` on the chapter when present. Each entry is a typed block (`why`, `comparison`, `bullets`, `tree`, …) with its own input shape. Unused types simply are not in the array — nothing is auto-added. When `blocks` is unset, legacy insight fields **and** `##` sections from `contentMarkdown` hydrate into blocks so lesson text is not a separate footer.
