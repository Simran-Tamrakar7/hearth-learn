...
Structured lessons with chapters. **Builtin manuals:** `playwright`, `cypress`, `testing-types`, `hearth-manual` (repository docs), `jmeter`, and `sql-qa`. Cypress is 108 chapters (Parts 0–14 + Appendices); rebuild from `scripts/cypress-manual-data/` via `node scripts/rebuild-cypress-manual.mjs`. JMeter is 43 chapters (Parts 0–11 + Appendices); rebuild from `scripts/jmeter-manual-data/` via `node scripts/rebuild-jmeter-manual.mjs`. SQL for QA is 69 chapters (Parts 0–12 + Appendices A–F); rebuild from `scripts/sql-qa-manual-data/` via `node scripts/rebuild-sql-qa-manual.mjs`. Then regenerate the index. Do not rewrite other manuals’ chapter files when changing one catalog.

- **Listing:** `src/app/manuals/registry.ts` (`KEPT_BUILTIN_SLUGS`). Those six always show on `/manuals`; localStorage hide is only for user-added manuals.
