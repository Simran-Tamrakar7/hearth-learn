...
- **Cypress builtin:** 108 chapters under Parts 0–14 plus Appendices (`types/cypress/`). Catalog JSON lives in `scripts/cypress-manual-data/`; `CYPRESS_TOC_VERSION` invalidates stale localStorage TOC. Catalog card title is **Cypress E2E Testing** (`/manuals/cypress`). Kept builtins cannot be hidden from this browser’s catalog.
- **Reader (all slugs):** Full Content / Summary / Quiz & Activities tabs; no AI Summary tab.
- **Full Content insights:** `blocks[]` via a `<BlockShell>` + registry (new types = Body + catalog entry). Per-row 1/2/3-column `blockLayout` with drag between rows/columns (`@dnd-kit`). Manual settings `allowedBlockTypes` filters the Add menu only. Incomplete blocks save as drafts. Legacy why/when/… fields still hydrate when `blocks` is unset; lesson `contentMarkdown` (`##` sections) hydrates as overview blocks so it is not a separate footer. Chapter titles that already include a number (`0.1`, `A.`) show that number once. Parts keep their stored index (Orientation is Part 0).
