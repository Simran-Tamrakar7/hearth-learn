...
| Topic | Detail |
|---|---|
| **Disk write target** | `types/<slug>/part-N/chapter-M.ts` — resolved via `chapters-manifest.ts` → `ManualChapter.sourceFile` |
...
| **Undo state** | `features/edit/editHistory.ts` — in-memory stack inside `ChapterContentEditor` (max 50 steps). Block add/delete/duplicate/reorder/layout use kind `"add"`; typing uses `"edit"`. Toolbar inserts in `LessonContentEditor` use `onAdd`. Lost on reload. Page-level TOC undo remains separate in `[slug]/page.tsx`. |
...
├── toc.ts
├── meta.json
├── chapters-manifest.ts   ← generated (chapterPaths + chapters[])
