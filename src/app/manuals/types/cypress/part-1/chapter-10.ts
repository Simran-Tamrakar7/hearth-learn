...
export const chapter = {
  ...
  "blocks": [
    ...
    },
    {
      ...
      "rows": [
        [
          "`npm i -g cypress` / `npm i cypress --save`",
          "`npm i -D cypress` + `npx cypress`"
        ],
        [
          "`cypress.json` + `integration/*.spec.js`",
          "`cypress.config.*` + `e2e/*.cy.ts`"
        ],
        [
          "Commit `videos/` or `cypress.env.json`",
          "gitignore both"
        ],
        [
          "`defaultCommandTimeout: 30000`",
          "Fix the wait; use per-command `{ timeout }`"
        ],
        [
          "`await cy.visit`",
          "Queue + `.should`"
        ...
        [
          "Assume videos always record",
          "Cypress **13** default `video: false`"
        ],
        [
          "Login in `before` only, rely on leftover cookies",
          "`testIsolation: true` → `beforeEach` / `cy.session`"
        ],
        [
          "`cy.task` returns a class instance",
          "Plain object / `null`"
        ]
      ],
      "caption": "Anti-checklist (if you did any of these, undo them)",
      "order": 1
    },
    {
      "id": "cy-1-10-md-2",
    ...
    },
    {
      ...
      "rows": [
        [
          "0 specs found",
          "Rename to `.cy.ts` / check `e2e/` not `integration/`"
        ],
        [
          "`ERR_CONNECTION_REFUSED`",
          "Start HRM; fix `baseUrl`"
        ],
        [
          "`cypress.env.json` in the PR",
          "gitignore; rotate if it had secrets"
        ],
        [
          "`await cy.visit`",
          "Delete await; use `.should`"
        ],
        [
          "Videos missing after v13",
          "Set `video: true` if you want them"
        ],
        [
          "Second test logged out",
          "`beforeEach` login; isolation is on"
        ],
        [
          "`task` undefined return",
          "`return null`"
        ],
        [
          "Config `baseUrl` ignored in CI",
          "`--config baseUrl=` or `return config` after mutate"
        ]
      ],
      "caption": "Common first-run errors (cheat sheet)",
      "order": 16
    },
    {
      "id": "cy-1-10-md-17",
