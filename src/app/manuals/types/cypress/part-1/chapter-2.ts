...
export const chapter = {
  "id": "cy-1-2-installation-environment-setup",
  "title": "1.2 Installation & Environment Setup",
  "minutes": 32,
  "level": "beginner",
  "phase": "Part 1 · Setup & Project Structure",
  "partName": "Part 1 · Setup & Project Structure",
  "overviewText": "Install Cypress as a project devDependency with npm (or pnpm/yarn), then let the first `cypress open` or `cypress verify` download the ~250MB Electron/binary cache. Pin the version in package.json, add npm scripts, and never install globally or into production dependencies. This chapter walks a Bizlevate HRM repo from empty Cypress to a verified binary, including Node/OS prerequisites, CI cache keys, and the Cypress 10+ config wizard.",
  "why": "Wrong install flags are the most expensive first-week mistake: global Cypress drifts from the lockfile; a production dependency bloats deploy artifacts; skipping verify in CI means the binary downloads during the test job and flakes on network. Interviewers ask how you install Cypress 'properly' because it reveals whether you have run it in CI.",
  "when": "Creating a new Cypress suite, onboarding a laptop, writing the CI install step, or recovering from 'Cypress failed to start' after a version bump.",
  "practical": {
    "app": "Bizlevate HRM",
    "scenario": "A new engineer clones the HRM repo on a clean macOS machine and must get Cypress launching against local HRM without polluting global npm or production Docker.",
    "pass": "They run npm i -D cypress (or npm ci), npx cypress verify, then npx cypress open from the repo; they refuse -g and --save; they know the binary cache is outside node_modules.",
    "fail": "They npm install -g cypress, or commit the Electron cache, or add Cypress to dependencies so 'CI can find it'."
  },
  "tools": [],
    ...
    },
    {
      ...
      "rows": [
        [
          ...
          "Global not on PATH, or you forgot `npx` / npm script"
        ],
        [
          "Verify fails SSL",
          "Proxy / intercepting antivirus"
        ],
        [
          "Verify every CI run downloads 250MB",
          "Cache path not saved (`~/.cache/Cypress` vs macOS Library path)"
        ],
        [
          "\"Cypress failed to start\" after bump",
          "Cache for old version; new version not verified"
        ],
        [
          "App Docker build includes Cypress",
          "`--save` / `dependencies` instead of `-D`"
        ]
      ],
      "caption": "Failure catalog",
      "order": 14
    },
    {
      "id": "cy-1-2-md-15",
  ...
  "limitations": [
    "1.2 Installation & Environment Setup is this Part's slice only; later chapters go deeper rather than repeating this one."
  ],
  "exercises": [],
  "resourceLinks": [],
  "steps": [],
