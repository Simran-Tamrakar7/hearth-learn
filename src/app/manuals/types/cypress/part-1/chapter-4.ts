...
export const chapter = {
  "id": "cy-1-4-first-test",
  "title": "1.4 First Test",
  "minutes": 30,
  "level": "beginner",
  "phase": "Part 1 · Setup & Project Structure",
  "partName": "Part 1 · Setup & Project Structure",
  "overviewText": "Your first Cypress spec is a describe/it that cy.visit()s an HRM URL and asserts with .should(). The test function runs synchronously and only enqueues commands; Cypress executes that queue afterward. You cannot await cy.* or assign cy.get() to a string variable. This chapter writes a passing smoke test, shows the equivalent Playwright and Selenium, and catalogs the first-week bugs that come from treating Cypress like Promises.",
  "why": "The queue-versus-await confusion is the most common first-week Cypress bug and the fastest way to fail a live-coding screen. A first test that 'works' with cy.wait(5000) teaches the wrong lesson; a first test that uses visit + get + should teaches retry-ability.",
  "when": "Immediately after folders exist and baseUrl (or a full URL) is known. Revisit the first time you reach for await cy.get or const el = cy.get('.btn').",
  "practical": {
    "app": "Bizlevate HRM",
    "scenario": "Write a smoke spec that opens /login and proves the Sign in heading is visible, then explain to a Playwright-experienced teammate why the test body has no await.",
    "pass": "You write cy.visit('/login'); cy.contains('Sign in').should('be.visible'); you explain enqueue-then-execute; you do not await cy.visit or store cy.get in a const.",
    "fail": "You write await cy.get('h1') or const text = cy.get('h1').text() and wonder why text is an object, or you sprinkle cy.wait(3000)."
  },
  "tools": [],
  ...
  "limitations": [
    "1.4 First Test is this Part's slice only; later chapters go deeper rather than repeating this one."
  ],
  "exercises": [],
  "resourceLinks": [],
  "steps": [],
