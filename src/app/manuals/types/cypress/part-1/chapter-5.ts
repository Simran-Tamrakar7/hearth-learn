...
/** 1.5 cypress.config.js/ts */
export const chapter = {
  "id": "cy-1-5-cypress-config-js-ts",
  "title": "1.5 cypress.config.js/ts",
  "minutes": 34,
  "level": "beginner",
  "phase": "Part 1 · Setup & Project Structure",
  "partName": "Part 1 · Setup & Project Structure",
  "overviewText": "Cypress 10 replaced cypress.json with a Node-evaluated cypress.config.js or cypress.config.ts that exports defineConfig({ e2e: { ... } }). This is where baseUrl, specPattern, timeouts, video, and testIsolation live. Timeouts are layered; you raise the specific timeout that matches the wait, never the global default as a flake blanket. Video defaults to false since Cypress 13, testIsolation defaults to true since Cypress 12, and setupNodeEvents replaced cypress/plugins/index.js.",
  "why": "Config is how a suite stays honest: baseUrl keeps specs portable, layered timeouts keep failures meaningful, and knowing the v10/v12/v13 defaults prevents cargo-cult copy-paste from old blog posts. Interviewers will ask what you put in cypress.config and what you refuse to put there.",
  "when": "After the first spec exists; whenever CI disagrees with local; when migrating from Cypress 9; when videos vanish after a Cypress 13 upgrade or tests leak state after a Cypress 12 upgrade.",
  "practical": {
    "app": "Bizlevate HRM",
    "scenario": "You add Cypress to the HRM app serving http://localhost:3000, want specs to say cy.visit('/login'), and must not hide SPA slowness by setting defaultCommandTimeout to 30 seconds.",
    "pass": "You set baseUrl, leave defaultCommandTimeout at 4s, use pageLoadTimeout/requestTimeout where appropriate, set video explicitly if CI needs recordings, and leave testIsolation true with beforeEach login later.",
    "fail": "You recreate cypress.json, or set defaultCommandTimeout: 30000 globally, or assume videos still record after Cypress 13 without video: true."
  },
  "tools": [],
  ...
  "limitations": [
    "1.5 cypress.config.js/ts is this Part's slice only; later chapters go deeper rather than repeating this one."
  ],
  "exercises": [],
  "resourceLinks": [],
  "steps": [],
