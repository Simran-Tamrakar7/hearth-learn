...
/** 1.8 TypeScript Setup */
export const chapter = {
  "id": "cy-1-8-typescript-setup",
  "title": "1.8 TypeScript Setup",
  "minutes": 26,
  "level": "beginner",
  "phase": "Part 1 · Setup & Project Structure",
  "partName": "Part 1 · Setup & Project Structure",
  "overviewText": "Cypress specs can be .cy.ts once a dedicated tsconfig.json lives inside cypress/ (not only at the repo root). That file sets compilerOptions.types to cypress, includes the e2e and support globs, and avoids colliding with Next.js/DOM libs. cypress.config.ts is a separate Node tsconfig problem, and custom commands need a Cypress.Chainable interface merge. This chapter gets HRM to a typed first spec without installing a second test runner.",
  "why": "Root tsconfig that excludes cypress/, missing types, and custom commands that are any are the three TS pain points. Interviewers distinguish people who dropped Cypress into a Next app from people who isolated the Cypress compiler options on purpose.",
  "when": "When you rename the first spec to .cy.ts, when VS Code highlights cy as unknown, and when you add Cypress.Commands.add('login').",
  "practical": {
    "app": "Bizlevate HRM",
    "scenario": "The HRM app already has a Next.js tsconfig. You want leave-request.cy.ts with typed cy.login later, without breaking next build.",
    "pass": "You add cypress/tsconfig.json with types: ['cypress'], include e2e and support, use cypress.config.ts via defineConfig, and declare custom commands in a .d.ts merged into Chainable.",
    "fail": "You only rename to .ts and hope, or you add Cypress to the app's types and break Next, or you leave Commands.add untyped."
  },
  "tools": [],
  ...
  "limitations": [
    "1.8 TypeScript Setup is this Part's slice only; later chapters go deeper rather than repeating this one."
  ],
  "exercises": [],
  "resourceLinks": [],
  "steps": [],
