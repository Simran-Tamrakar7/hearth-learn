  ...
  "blocks": [
    {
      "id": "cy-11-1-md-0",
      "type": "overview",
      "heading": "GitHub Actions — the pattern to memorize",
      "content": "```yaml\nname: cypress-smoke\non: [pull_request]\njobs:\n  smoke:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: cypress-io/github-action@v6\n        with:\n          browser: chrome\n          start: npm run start\n          wait-on: 'http://localhost:3000'\n          spec: cypress/e2e/**/*.cy.ts\n        env:\n          CYPRESS_grepTags: '@smoke'\n          CYPRESS_BASE_URL: 'http://localhost:3000'\n          CYPRESS_apiUrl: 'http://localhost:4000'\n```\n\nWhat the action does: npm ci (or yarn), cache the Cypress binary, run `start` in the background, poll `wait-on` until the HRM UI responds, then `cypress run --browser chrome`. If you already deploy staging, drop `start`/`wait-on` and set `CYPRESS_BASE_URL` to staging (10.2) with secrets for users.",
      "order": 0
    },
    {
    ...
    },
    {
      ...
      "type": "overview",
      "heading": "Jenkins",
      "content": "A pipeline stage with the same shell, or a Docker agent `cypress/included` so `cypress` is on PATH. Inject credentials as masked env (`CYPRESS_`). Do not store `cypress.env.json` on the agent disk with prod passwords.",
      "order": 2
    },
    {
      "id": "cy-11-1-md-3",
      "type": "overview",
      "heading": "Parallel and browsers",
      "content": "Two browsers = two jobs (or a matrix). Spec sharding = matrix of spec globs or Cloud `--parallel` (9.8, 11.3). One step cannot \"run Chrome and Firefox in parallel inside Cypress.\"",
      "order": 3
    },
    {
      "id": "cy-11-1-md-4",
      ...
      "content": "Playwright's GitHub action / `npx playwright install --with-deps` is the cousin. Selenium needs drivers or a Grid — more moving parts. Cypress's official action is a genuine DX advantage; still pin action major versions.\n\nInterview line: \"I use `cypress-io/github-action` with `start` + `wait-on` + `browser: chrome`. CI is `cypress run`. Smoke is `CYPRESS_grepTags`.\"",
      "order": 4
    }
  ],
  "advantages": [
