  ...
  "blocks": [
    {
      "id": "cy-11-4-md-0",
      "type": "overview",
      "heading": "What is real vs adjacent",
      "content": "**Real, paid, Cypress-branded:** Cloud Test Replay / Debug (reconstruct a recorded run), analytics that group failures, and whatever \"AI\" assistant the current Cloud plan wraps around those artifacts. These need `--record`.\n\n**Not real in OSS Cypress:** a Trace Viewer, self-healing `cy.get('.css-x7k')` that silently retargets, or an offline zip you open with `show-trace`.\n\nIf the feature is not reproducible from `cypress run` without a key, it is Cloud, not Cypress-the-test-runner.",
      "order": 0
    },
    {
      "id": "cy-11-4-md-1",
      "type": "overview",
      "heading": "How to use it without lying to yourself",
      "content": "1. Record CI (11.3).\n2. Open the failed run; watch Replay if you have it.\n3. Fix the spec: intercept, `data-cy`, `cy.origin` args (9.1), isolation.\n4. If an AI panel suggests a selector, treat it like Selector Playground — a guess, then apply Part 4 discipline.\n\nGenerated Studio / AI specs will not invent `{ args }` for Okta. Review like a junior PR.",
      "order": 1
    },
    {
      "id": "cy-11-4-md-2",
      "type": "overview",
      "heading": "Versus Playwright and Selenium vendors",
      "content": "Playwright's OSS trace is already a powerful post-mortem **without** AI. mabl, Testim, and some Selenium clouds have sold \"self-healing\" for years; it trades determinism for magic. Cypress Cloud AI is in that product category. Appium device clouds also sell analytics — still not a substitute for `data-cy`.\n\nInterview line: \"Cloud AI is dashboard assistance on recorded runs. The runner has no Trace Viewer and no self-healing I would bet payroll on. Playwright traces and Selenium vendor videos are the cousins.\"",
      "order": 2
    }
  ],
  "advantages": [
