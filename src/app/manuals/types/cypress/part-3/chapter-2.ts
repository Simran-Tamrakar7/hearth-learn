...
export const chapter = {
  ...
  why: "Mastering Forms & Inputs in Cypress's command-queue model prevents flaky specs and wrong Playwright ports.",
  when: "Read when implementing or debugging forms & inputs in your suite.",
  practical: { app: "Web application under test", scenario: "Spec fails around forms & inputs — need Cypress-native pattern.", pass: "Apply chapter patterns with retry semantics not bare cy.wait(ms).", fail: "Port Playwright await code or fixed delays." },
  advantages: ["multi-field chains","validation assertions","disabled submit tests","cy.select native","file deferred Ch19","reset patterns"],
  limitations: ["dynamic rows need scope","controlled inputs invoke val","async blur timing","wizards need sync","password field quirks","i18n breaks text"],
  tools: [],
