...
export const chapter = {
  ...
  why: "Mastering iframes in Cypress's command-queue model prevents flaky specs and wrong Playwright ports.",
  when: "Read when implementing or debugging iframes in your suite.",
  practical: { app: "Web application under test", scenario: "Spec fails around iframes — need Cypress-native pattern.", pass: "Apply chapter patterns with retry semantics not bare cy.wait(ms).", fail: "Port Playwright await code or fixed delays." },
  advantages: ["frameLoaded wait","iframe plugin entry","same vs cross origin","nested traversal","parent return pattern","Playwright frame_locator note"],
  limitations: ["cross-origin needs origin","Stripe maps cross-origin","plugin on older versions","timing without frameLoaded","no shadow+iframe combo","heavy iframes suggest Playwright"],
  tools: [],
