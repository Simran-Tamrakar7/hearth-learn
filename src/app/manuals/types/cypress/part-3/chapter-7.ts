  ...
  "blocks": [
    {
      "id": "cy-3-7-md-0",
      "type": "overview",
      "heading": "`.selectFile()` since 9.3",
      "content": "For years Cypress had no upload command; `cypress-file-upload` (`.attachFile()`) was universal. **Cypress 9.3 added `.selectFile()` to core.** New tests should use the built-in. Recognize `.attachFile` in old code; do not add the plugin on a modern Cypress just because a blog post says so.\n\n```js\ncy.get('[data-cy=resume-upload]').selectFile('cypress/fixtures/resume.pdf');\n\ncy.get('[data-cy=attachments]').selectFile([\n  'cypress/fixtures/id-front.png',\n  'cypress/fixtures/id-back.png',\n]);\n```\n\nPaths are from the project root (where `cypress.config` lives), not from the spec file.",
      "order": 0
    },
    {
      "id": "cy-3-7-md-1",
      "type": "overview",
      "heading": "Drop-zones vs file inputs",
      "content": "Default action: set files on an `<input type=\"file\">` (including `display:none` inputs — this is a legitimate `{ force: true }` / hidden-input case; `.selectFile` can target the input directly).\n\nIf the UI is a big \"drop files here\" surface whose JS listens for `drop` and **does not** wire a hidden input, the default action never runs the app code:\n\n```js\ncy.get('[data-cy=receipt-dropzone]').selectFile(\n  'cypress/fixtures/taxi-receipt.png',\n  { action: 'drag-drop' },\n);\n```\n\n`action: 'select'` (default) vs `'drag-drop'` is the difference between setting `input.files` and dispatching drag events. Match the widget.",
      "order": 1
    },
    {
      "id": "cy-3-7-md-2",
      "type": "overview",
      "heading": "In-memory files",
      "content": "```js\ncy.get('[data-cy=csv-import]').selectFile({\n  contents: Cypress.Buffer.from('employeeId,name\\n1,Simran Tamrakar\\n'),\n  fileName: 'employees.csv',\n  mimeType: 'text/csv',\n});\n```\n\nUseful when you do not want a fixture on disk, or when the file must include today's date. `Cypress.Buffer` is the bundled buffer helper.",
      "order": 2
    },
    {
      ...
      "content": "Selecting a file is not a complete test. Assert the filename chip, a preview thumbnail, disabled Submit until upload finishes, or:\n\n```js\ncy.intercept('POST', '/api/employees/*/resume').as('resumeUpload');\ncy.get('[data-cy=resume-upload]').selectFile('cypress/fixtures/resume.pdf');\ncy.wait('@resumeUpload').its('response.statusCode').should('eq', 201);\n```",
      "order": 3
    },
    {
      "id": "cy-3-7-md-4",
      ...
      "content": "Playwright `locator.set_input_files()` is the analogue. Cypress's `{ action: 'drag-drop' }` is the extra knob for drop-zones. Both should use fixtures, not production PII files.",
      "order": 4
    }
  ],
  "advantages": [
