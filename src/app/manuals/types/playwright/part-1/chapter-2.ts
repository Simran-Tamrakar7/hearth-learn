...
export const chapter = {
  ...
  advantages: ["venv isolation prevents cross-project version conflicts","Two-step install clearly separates Python package from browsers","playwright install pins version-matched browser binaries","pytest --headed verifies setup with visible browser","Mac/Linux and Windows activation commands documented","Smoke test confirms end-to-end toolchain before Chapter 12"],
  limitations: ["Browser download is large — slow on first install","Corporate proxies may block playwright install CDN","Python version minimum must be verified against Playwright release","venv habit is new for developers from global pip installs","CI agents need separate playwright install --with-deps on Linux","Windows path escaping trips up first-time pytest runs"],
  tools: [],
